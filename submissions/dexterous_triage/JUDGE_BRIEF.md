...
Dexterous Triage Lab is a five-finger, 22-channel MuJoCo dexterous-hand system for a long-horizon medication triage task. It is designed to answer the previous review concern directly: the current version is not only a staged video, it logs closed-loop residual control, raw-vs-corrected visual servo error, contact balancing, a five-finger contact timeline, slip recovery, concise subtitles, dynamic route/residual/slip visuals, key-moment markers, a real-world physical-test protocol, and fixed-seed stress-test results.

## What To Inspect First

1. `artifacts/dexterous_triage_demo.mp4` - generated demo video.
2. `artifacts/dexterous_triage_real_world_test_plan.json` - hardware transfer protocol, safety limits, and physical test matrix.
3. `artifacts/dexterous_triage_report.json` - success criteria, closed-loop metrics, self-audit.
4. `artifacts/dexterous_triage_eval.json` - 32-rollout stress test and baseline comparison.
5. `artifacts/dexterous_triage_policy_card.json` - policy inputs, outputs, topology, and evidence.
6. `artifacts/dexterous_triage_narration.srt` - stage narration matching the video overlays.
7. `artifacts/dexterous_triage_contact_timeline.json` - five-finger active-contact, balance, stable-hold, and recovery evidence.
8. `dexterous_triage_scene.xml` - five-finger MJCF hand, actuators, sensors, free bodies, and task objects.
...
- Real-world test protocol: four hardware test cases, calibration steps, force/speed safety limits, and telemetry evidence requirements
...
- Presentation: generated video overlays concise stage text, route trails, scan beams, residual arrows, slip ripples, key-moment markers, servo error, slip observer, residual norm, and confidence; SRT subtitles and contact timeline evidence are included.
