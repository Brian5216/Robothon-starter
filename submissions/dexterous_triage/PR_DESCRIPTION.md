...
- Technical approach: Deterministic imitation-style stage prior plus closed-loop residual policy using visual-servo/contact/slip feedback, with MJCF joints, actuators, free bodies, collision geometry, frame sensors, touch sensors, generated narrative video overlays, SRT subtitles, extracted keyframe screenshots, trajectory JSON, five-finger contact timeline, policy card, fixed-seed stress evaluation, and self-audit report.
- Core features: Five-finger grasp, in-hand cap rotation, fragile-object handling, audit button confirmation, slip recovery, raw-vs-corrected visual-servo metrics, per-sample residual actions, controller confidence, machine-readable contact timeline, three extracted high-moment screenshots, 32-rollout baseline-vs-residual evaluation, and data-collection export.
...
- v6 highlights: post-residual median visual-servo error is 0.00672 m, down 68.61% from raw error; demo video uses concise subtitles plus dynamic route trails, residual arrows, scan beams, cap-rotation arcs, and slip-recovery ripples; extracted keyframes make the three highest-scoring visual moments easy to inspect; the contact timeline exposes five-finger active contact, stable hold, and recovery-window evidence.
...
- [x] Keyframe screenshots are included: `submissions/dexterous_triage/artifacts/keyframes/`
