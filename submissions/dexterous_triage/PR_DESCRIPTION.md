...
- Technical approach: Deterministic imitation-style stage prior plus closed-loop residual policy using visual-servo/contact/slip feedback, with MJCF joints, actuators, free bodies, collision geometry, frame sensors, touch sensors, generated narrative video overlays, one concise SRT subtitle layer, trajectory JSON, five-finger contact timeline, policy card, fixed-seed stress evaluation, self-audit report, and report-level physical bench protocol.
- Core features: Five-finger grasp, in-hand cap rotation, fragile-object handling, audit button confirmation, slip recovery, raw-vs-corrected visual-servo metrics, per-sample residual actions, controller confidence, machine-readable contact timeline, clear KEY 1/2/3 video beat markers, 32-rollout baseline-vs-residual evaluation, and data-collection export.
...
- v13 highlights: preserves the strongest 89.3 evidence package, keeps the 60s pacing, removes duplicate caption layers, keeps only concise SRT subtitles, upgrades key moments into clearer KEY 1/2/3 badges with light motion effects, and documents a real-world physical-test protocol honestly in the report.
