...
DEFAULT_REAL_WORLD_PLAN = DEFAULT_ARTIFACT_DIR / "dexterous_triage_real_world_test_plan.json"
...
def key_moment_for_phase(phase: float) -> str | None:
    if 0.29 <= phase <= 0.37:
        return "KEY: stable five-finger grasp"
    if 0.43 <= phase <= 0.54:
        return "KEY: cap rotation"
    if 0.82 <= phase <= 0.91:
        return "KEY: slip recovery"
    return None
...
def overlay_frame(frame: np.ndarray, sample: dict, frame_idx: int, total_frames: int) -> np.ndarray:
    ...
    key_moment = key_moment_for_phase(float(sample["phase"]))
    if key_moment:
        pulse = 0.5 + 0.5 * math.sin(frame_idx * 0.65)
        x1, y1, x2, y2 = width - 382, 14, width - 196, 36
        draw.rectangle((x1, y1, x2, y2), fill=(255, 210, 72, int(120 + 80 * pulse)), outline=(255, 245, 150, 230), width=1)
        draw.text((x1 + 8, y1 + 6), key_moment, fill=(10, 16, 22, 255), font=font)
...
def render_schematic(sample: dict, width: int, height: int) -> np.ndarray:
    ...
    if key_moment:
        pulse = int(8 + 12 * (0.5 + 0.5 * math.sin(phase * 180.0)))
        target_x, target_y = (cx, cy) if sample["stage"] == "uncap" else (vx, vy)
        draw.ellipse((target_x - 30 - pulse, target_y - 30 - pulse, target_x + 30 + pulse, target_y + 30 + pulse), outline=(255, 220, 90, 210), width=4)
        draw.rectangle((target_x + 34, target_y - 26, target_x + 204, target_y - 2), fill=(255, 210, 72, 205), outline=(255, 245, 150, 230), width=1)
        draw.text((target_x + 42, target_y - 20), key_moment.replace("KEY: ", ""), fill=(10, 16, 22, 255), font=ImageFont.load_default())
...
def real_world_test_plan(report: dict) -> dict:
    return {
        "project": "Dexterous Triage Lab",
        "purpose": "Physical bench protocol for transferring the MuJoCo vial-triage policy to a real five-finger hand without overstating that hardware tests were already run.",
        "status": "hardware protocol prepared; simulated acceptance metrics generated by this repository",
        "recommended_hardware": [
            "five-finger dexterous hand with thumb opposition and fingertip force sensing",
            "6-DoF wrist or gantry with at least xyz translation and wrist roll",
            "AprilTag or ArUco marker board around the vial tray for camera calibration",
            "thin medicine-vial surrogate with removable cap and compliant sterile-pod target",
            "force-limited emergency stop and 20 Hz telemetry logger",
        ],
        "calibration_steps": [
            "Calibrate camera-to-base transform with a 6x6 marker board; require reprojection error below 1.5 px.",
            "Map MuJoCo hand frame to hardware palm frame; require vial grasp-site pose error below 8 mm.",
            "Measure empty-grip fingertip offsets and set per-finger contact thresholds.",
            "Run the no-vial dry trajectory at 30% speed and verify no joint-limit or self-collision warnings.",
        ],
        "physical_test_matrix": [
            {"case": "nominal_vial_pick_uncap_place", "trials": 10, "pass_condition": ">= 8/10 full task completions"},
            {"case": "vial_pose_offset_15mm", "trials": 10, "pass_condition": "median final vial error <= 18 mm"},
            {"case": "cap_torque_variation", "trials": 8, "pass_condition": "no crush events and cap removed in >= 6/8 trials"},
            {"case": "induced_slip_recovery", "trials": 8, "pass_condition": "slip recovered within 1.5 s in >= 6/8 trials"},
        ],
        "hardware_safety_limits": {
            "max_fingertip_force_n": 8.0,
            "max_wrist_speed_mps": 0.12,
            "max_cap_rotation_speed_radps": 1.8,
            "abort_if_vial_error_exceeds_mm": 55.0,
        },
        "simulated_acceptance_metrics": report["closed_loop_metrics"],
        "evidence_to_collect_on_hardware": [
            "synchronized RGB video",
            "joint position and velocity logs",
            "fingertip force/contact logs",
            "vial pose estimate and final placement error",
            "slip recovery timestamp and residual action norm",
        ],
    }
...
def run_demo(
    *,
    ...
    real_world_plan_path.parent.mkdir(parents=True, exist_ok=True)
    ...
    hardware_plan = real_world_test_plan(report)
    ...
    real_world_plan_path.write_text(json.dumps(hardware_plan, indent=2), encoding="utf-8")
    ...
    return {
        ...
        "real_world_test_plan": str(real_world_plan_path),
        "duration_s": duration_s,
        "fps": fps,
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--real-world-plan", type=Path, default=DEFAULT_REAL_WORLD_PLAN)
...
def main() -> int:
    ...
    summary = run_demo(
        ...
        real_world_plan_path=args.real_world_plan,
        duration_s=duration,
        fps=fps,
        width=args.width,
        height=args.height,
