...
PROJECT_NAME = "Dexterous Reflex Bench"
...
def overlay_frame(frame: np.ndarray, sample: dict, frame_idx: int, total_frames: int) -> np.ndarray:
    ...
    draw.text((22, 16), f"{PROJECT_NAME} - 4ms tactile-reflex MuJoCo policy", fill=(238, 246, 255, 255), font=font)
...
def self_audit_report(trajectory: list[dict], duration_s: float, fps: int, policy_state: ResidualPolicyState) -> dict:
    ...
    return {
        "project": PROJECT_NAME,
        "generated_by": "submissions/dexterous_reflex_bench/run_dexterous_triage.py",
...
def policy_card(policy_state: ResidualPolicyState, report: dict) -> dict:
    return {
        "policy_name": "Dexterous Reflex Bench Policy v1",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description=f"Render the {PROJECT_NAME} MuJoCo submission.")
